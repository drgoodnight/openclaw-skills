#!/usr/bin/env bash
# index-simple.sh — Reliable indexer: extract, chunk (awk), embed, store
#
# Replaces the complex index-library.sh with a straightforward pipeline.
# Uses awk for chunking instead of bash string manipulation.
#
# Usage: index-simple.sh <library-dir> <data-dir> [--reindex]

set -euo pipefail

LIBRARY_DIR="${1:?Usage: index-simple.sh <library-dir> <data-dir> [--reindex]}"
DATA_DIR="${2:?Usage: index-simple.sh <library-dir> <data-dir> [--reindex]}"
REINDEX="${3:-}"
...
CHUNK_TARGET=1000
BATCH_SIZE=10
QDRANT_CONTAINER="clinical-tutor-qdrant"
...
# --- Ensure Ollama ---
curl -sf "${OLLAMA_URL}/api/tags" >/dev/null 2>&1 || { fail "Ollama not reachable."; exit 1; }
...
# --- Chunking via awk ---
# Splits text into ~CHUNK_TARGET char chunks on paragraph boundaries
...
chunk_file() {
  ...
  # Awk splits on paragraph boundaries, outputs raw text separated by null bytes.
  # Then jq handles all JSON escaping properly.
  local chunk_num=0

  awk -v target="$CHUNK_TARGET" '
  BEGIN { RS=""; chunk=""; clen=0 }
  {
    para = $0; gsub(/\r/, "", para); plen = length(para)
  ...
  ' "$text_file" | while IFS= read -r -d '' raw_chunk; do
...
while IFS= read -r -d '' filepath; do
  ...
  -o -iname '*.docx' -o -iname '*.md' -o -iname '*.txt' \
  \) -print0 | sort -z)
...
# Query Qdrant for all unique topic/source combinations
# Scroll through all points to collect metadata
SCROLL_OFFSET=""
ALL_PAYLOADS="[]"

while true; do
  if [ -z "$SCROLL_OFFSET" ]; then
    SCROLL_BODY='{"limit":100,"with_payload":{"include":["source","topic","has_images"]}}'
  else
    SCROLL_BODY="{\"limit\":100,\"offset\":${SCROLL_OFFSET},\"with_payload\":{\"include\":[\"source\",\"topic\",\"has_images\"]}}"
  fi
  ...
  SCROLL_OFFSET=$(echo "$SCROLL_RESP" | jq '.result.next_page_offset // empty' 2>/dev/null)
  [ -z "$SCROLL_OFFSET" ] && break
done

# Build registry from collected payloads
...
echo "Vectors: $GLOBAL_OFFSET total"
